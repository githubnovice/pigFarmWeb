...
! function(M, Y, t) {
	...
	function qa(b, a) {
		if(x(b)) {
			...
			for(c in a = a || {}, b)("$" !== c.charAt(0) || "$" !== c.charAt(1)) && (a[c] = b[c]);
		return a || b
	}
	...
	function Nd() {
		...
		uc || ((ra = M.jQuery) && ra.fn.on ? (B = ra, z(ra.fn, {
			...
			if(Pb) Pb = !1;
				...
				for(var e, d = 0; null != (e = a[d]); d++)(c = ra._data(e, "events")) && c.$destroy && ra(e).triggerHandler("$destroy");
			b(a)
		...
		if(a)
			if(a.nodeType) b[b.length++] = a;
				...
				if("number" == typeof c && a.window !== a) {
					if(c)
						for(var d = 0; c > d; d++) b[b.length++] = a[d]
				} else b[b.length++] = a
			}
	}
	...
	function pf(b, a, c, d) {
		...
		m.cookies = function(a, b) {
			...
			if(!a) {
				if(p.cookie !== y)
					for(y = p.cookie, d = y.split("; "), ea = {}, f = 0; f < d.length; f++) e = d[f], g = e.indexOf("="), g > 0 && (a = k(e.substring(0, g)), ea[a] === t && (ea[a] = k(e.substring(g + 1))));
				return ea
			}
			...
			return Ma(a, "directive"), F(a) ? (Qb(e, "directiveFactory"), d.hasOwnProperty(a) || (d[a] = [], b.factory(a + "Directive", ["$injector", "$exceptionHandler", function(b, e) {
				...
				return s(d[a], function(d, g) {
					try {
						...
						G(h) ? h = {
							compile: da(h)
						} : !h.compile && h.link && (h.compile = da(h.link)), h.priority = h.priority || 0, h.index = g, h.name = h.name || a, h.require = h.require || h.controller && h.name, h.restrict = h.restrict || "EA", H(h.scope) && (h.$$isolateBindings = c(h.scope, h.name)), f.push(h)
					} catch(l) {
						e(l)
					}
				}), f
		...
		this.debugInfoEnabled = function(a) {
			...
			function E(a, b, c, d, e) {
				...
				return function(b, c, d) {
					...
					if(d = d.futureParentElement, e && e.$$boundTransclude && (e = e.$$boundTransclude), g || (g = (d = d && d[0]) && "foreignobject" !== ua(d) && d.toString().match(/SVG/) ? "svg" : "html"), d = "html" !== g ? B(Wb(g, B("<div>").append(a).html())) : c ? La.clone.call(a) : a, h)
			...
			function ea(a, d, e, f, g, l, k, p, r) {
				function w(a, b, c, d) {
					a && (c && (a = I(a, c, d)), a.require = K.require, a.directiveName = z, (S === K || K.$$isolateScope) && (a = Z(a, {
						isolateScope: !0
					})), k.push(a)), b && (c && (b = I(b, c, d)), b.require = K.require, b.directiveName = z, (S === K || K.$$isolateScope) && (b = Z(b, {
						isolateScope: !0
					})), p.push(b))
				}
				...
				for(var P, X, K, z, N, Q, A = -Number.MAX_VALUE, J = r.controllerDirectives, S = r.newIsolateScopeDirective, ka = r.templateDirective, ea = r.nonTlbTranscludeDirective, ca = !1, D = !1, C = r.hasElementTranscludeDirective, aa = e.$$element = B(d), Aa = f, M = 0, R = a.length; R > M; M++) {
					...
					if((N = K.scope) && (K.templateUrl || (H(N) ? (Oa("new/isolated scope", S || P, K, aa), S = K) : Oa("new/isolated scope", S, K, aa)), P = P || K), z = K.name, !K.templateUrl && K.controller && (N = K.controller, J = J || {}, Oa("'" + z + "' controller", J[z], K, aa), J[z] = K), (N = K.transclude) && (ca = !0, K.$$tlb || (Oa("transclusion", ea, K, aa), ea = K), "element" == N ? (C = !0, A = K.priority, N = aa, aa = e.$$element = B(Y.createComment(" " + z + ": " + e[z] + " ")), d = aa[0], V(g, Za.call(N, 0), d), Aa = E(N, f, A, l && l.name, {
							nonTlbTranscludeDirective: ea
						})) : (N = B(Tb(d)).contents(), aa.empty(), Aa = E(N, f))), K.template)
			...
			Xb.prototype = {
				...
				$set: function(a, b, d, e) {
					var f = this.$$element[0],
						...
						h = kf(f, a),
						f = a;
					if(g ? (this.$$element.prop(a, b), e = g) : h && (this[h] = b, f = h), this[a] = b, e ? this.$attr[a] = e : (e = this.$attr[a]) || (this.$attr[a] = e = tc(a, "-")), g = ua(this.$$element), "a" === g && "href" === a || "img" === g && "src" === a) this[a] = b = A(b, "src" === a);
					else if("img" === g && "srcset" === a) {
						for(var g = "", h = U(b), l = /(\s+\d+x\s*,|\s+\d+w\s*,|\s+,|,\s+)/, l = /\s/.test(h) ? l : /(,)/, h = h.split(l), l = Math.floor(h.length / 2), k = 0; l > k; k++) var q = 2 * k,
							g = g + A(U(h[q]), !0),
							g = g + (" " + U(h[q + 1]));
		...
		this.digestTtl = function(a) {
			...
			l.prototype = {
				...
				$watch: function(a, b, d) {
					...
					return c = null, G(b) || (h.fn = C), f || (f = this.$$watchers = []), f.unshift(h),
						function() {
							Xa(f, h), c = null
						}
				},
	...
	function nd(b, a, c, d, e) {
		...
		if(k) e > 0 && 1 > b && (h = b.toFixed(e), b = parseFloat(h));
		else {
			...
			var g = ("" + b).split(od),
				...
				u = a.gSize;
			if(k.length >= q + u)
				for(p = k.length - q, m = 0; p > m; m++) 0 === (p - m) % u && 0 !== m && (h += c), h += k.charAt(m);
	...
	function Gf() {
		return function(b, a) {
			...
			var c, d;
			if(a > b.length ? a = b.length : a < -b.length && (a = -b.length), a > 0) c = 0, d = a;
			else {
				if(!a) return [];
		...
		readString: function(a) {
			...
			for(var d = "", e = a, f = !1; this.index < this.text.length;) {
				var g = this.text.charAt(this.index),
					e = e + g;
				if(f) "u" === g ? (f = this.text.substring(this.index + 1, this.index + 5), f.match(/[\da-f]{4}/i) || this.throwError("Invalid unicode escape [\\u" + f + "]"), this.index += 4, d += String.fromCharCode(parseInt(f, 16))) : d += Xf[g] || g, f = !1;
				else if("\\" === g) f = !0;
				else {
					...
					return u ? n = a(u) : (w = function(a, c) {
						...
						function(a, f, g, k, m) {
							...
							a.$watchCollection(p, function(g) {
								var k, p, E, C, S, N, G, J, x, H, n = f[0],
									z = ha();
								if(q && (a[q] = g), Ta(g)) J = g, p = v || w;
								else {
									p = v || D, J = [];
								...
								for(E in u) {
									if(x = u[E], G = tb(x.clone), c.leave(G), G[0].parentNode)
										for(k = 0, p = G.length; p > k; k++) G[k].$$NG_REMOVED = !0;
									x.scope.$destroy()
								}
								for(k = 0; C > k; k++)
					...
					function p(e, f, g) {
						...
						function p() {
							...
							var d, l, n, r, t, a = {
								...
								c = [""];
							n = g.$viewValue, r = P(e) || [];
							var x, A, D, F, B = H ? Object.keys(r).sort() : r,
								N = {};
							t = k(n);
							var U, V, I = !1;
